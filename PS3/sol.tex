...
\begin{enumerate}
    ...
    \item[(d)] Figure~\ref{fig:gmm_unsup_0}, Figure~\ref{fig:gmm_unsup_1}, and
    Figure~\ref{fig:gmm_unsup_2} show the unsupervised GMM cluster assignments
    obtained from three independent random initializations. The clusters
    recovered vary noticeably across trials, illustrating that unsupervised
    EM is sensitive to initialization and can converge to different local
    optima of the log-likelihood.
    ...
    \item[(e)] Figure~\ref{fig:gmm_ss_0}, Figure~\ref{fig:gmm_ss_1}, and
    Figure~\ref{fig:gmm_ss_2} show the semi-supervised GMM cluster assignments
    for the same three initializations, using a small number of labeled
    examples. Compared to the unsupervised results in part (d), the
    semi-supervised clusters are much more consistent across trials and
    converge in fewer iterations, since the labeled examples anchor each
    Gaussian component to the correct cluster from the start.
