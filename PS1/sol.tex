...
\begin{homeworkProblem}
    ...
    \]
    where $y^{(i)}\in\{0,1\}$, $h_\theta(x)=g(\theta^Tx)$ and $g(z)=1/(1+e^{-z})$. The Hessian, $\nabla^2_\theta J(\theta)\in \R^{n\times n}$, has entries
    \[
        \left(\nabla^2_\theta J(\theta)\right)_{ij}
        =\frac{\partial^2J(\theta)}{\partial\theta_i\partial\theta_j}
        =\frac{\partial}{\partial\theta_j}\left(
            -\frac{1}{m}\sum^m_{k=1}{
                \frac{y^{(k)}}{h_\theta(x^{(i)})}\cdot
                \frac{\mb dg(\theta^Tx^{(i)})}{\mb d(\theta^Tx^{(i)})}\cdot
                x_i+
                \frac{1-y^{(k)}}{1-h_\theta(x^{(i)})}\cdot
                \left(
                    -\frac{\mb dg((\theta^Tx^{(i)}))}{\mb d(\theta^Tx^{(i)})}
                    \cdot x_i
                \right)
            }
        \right).
    \]
     As $g'(z)=g(z)(1-g(z))$, therefore
     \[
     \begin{aligned}
        H_{ij}
        &=\frac{\partial}{\partial\theta_j}\left(
            -\frac{1}{m}\sum^m_{k=1}{
                \frac{y^{(k)}}{h_\theta(x^{(i)})}\cdot
                h_\theta(x^{(k)})(1-h_\theta(x^{(k)})x_i+
                \frac{1-y^{(k)}}{1-h_\theta(x^{(k)})}\cdot
                \left(
                    -h_\theta(x^{(k)})(1-h_\theta(x^{(k)})x_i
                \right)
            }
        ...
        &=\frac{\partial}{\partial\theta_j}\left(
            -\frac{1}{m}\sum^m_{k=1}{
                y^{(k)}(1-h_\theta(x^{(k)})x_i-
                (1-y^{(k)})h_\theta(x^{(k)})x_i
            }
        \right)\\
        &=\frac{\partial}{\partial\theta_j}\left(
            \frac{1}{m}\sum^m_{k=1}{
                (h_\theta(x^{(k)})-y^{(k)})x^{(k)}_i
            }
        \right)\\
        &=\frac{1}{m}\sum^m_{k=1}{
                h_\theta(x^{(k)})(1-h_\theta(x^{(k)}))x^{(k)}_ix^{(k)}_j
            }\\
        &=\frac{1}{m}\sum^m_{k=1}{
                h_\theta(x^{(k)})(1-h_\theta(x^{(k)}))\left[(x^{(k)}x^{(k)})^T\right]_{ij}
            }
    \end{aligned}
    \]
    Hence,
    \[
        H=\frac{1}{m}\sum^m_{k=1}{
                h_\theta(x^{(k)})(1-h_\theta(x^{(k)}))\left[x^{(k)}(x^{(k)})^T\right]
            }.
    \]
    ...
    \frac{\partial}{\partial\mu_0}\ell(\phi,\mu_0,\mu_1,\Sigma)
     &=\frac{\partial}{\partial\mu_0}\sum_{i=1}^m{
    ...
    w.r.t. $\Sigma$, consider it as a real number:
