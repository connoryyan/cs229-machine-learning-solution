...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    alpha = learning_rate * (y_i - predict(state, kernel, x_i))
...
def train_perceptron(kernel_name, kernel, learning_rate):
    ...
    """
    train_x, train_y = util.load_csv('./PS2/data/ds5_train.csv')
    ...
    test_x, test_y = util.load_csv('./PS2/data/ds5_test.csv')

    plt.figure(figsize=(12, 8))
    util.plot_contour(lambda a: predict(state, kernel, a))
    util.plot_points(test_x, test_y)
    plt.savefig('./PS2/output/p05_{}_output.png'.format(kernel_name))

    predict_y = [predict(state, kernel, test_x[i, :]) for i in range(test_y.shape[0])]

    np.savetxt('./PS2/output/p05_{}_predictions'.format(kernel_name), predict_y)
